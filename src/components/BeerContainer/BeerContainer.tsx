...
const BeerContainer = ({searchBeers, sideNavToggled}: BeerContainerProps) => {
    ...
    <div className="beer-tiles-container">
      {searchBeers.map((beer) => (
        <BeerTiles
          ...
          name={beer.name}
          description={beer.description} tagline={beer.tagline} brewDate={beer.first_brewed} abv={beer.abv} ph={beer.ph} foodPairing={beer.food_pairing} sideNavToggled={sideNavToggled} beerID={beer.id}    />
      ))}
    </div>
  );
}
